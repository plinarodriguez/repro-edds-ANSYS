...
outdir = "figures/species_transport/TU/dosage"
csv_outdir = "figures/species_transport/TU/dosage"
...
u_therm = 0.55 #0.42      # K; placeholder within 0.2--0.6 K suggested range
...
##### new analysis
# ============================================================
# Statistical test proposed in the email
# ============================================================
# The email recommendation is interpreted here as:
#   1) compute temperature-weighted flux histories,
#   2) show experimental flux uncertainty envelopes,
#   3) perform one statistical test on the resulting integrated dose values.
#
# The dose values are the time integrals of the flux histories, so this is the
# cleanest single t-test associated with the flux analysis.
#
# H0: mean experimental dose equals mean simulation dose
# HA: mean experimental dose differs from mean simulation dose
#
# This is NOT an equivalence test. If p > 0.05, write:
# "failed to reject equal means" or "not statistically distinguishable."
# If p < 0.05, write:
# "statistically distinguishable."

# alpha = 0.05

# # Welch two-sample t-test is preferred because n_exp != n_sim and variances may differ.
...
# # 95% confidence intervals on the mean dose distributions.
# exp_mean, exp_ci_low, exp_ci_high = mean_ci_t(D_exp_all, confidence=confidence)
# sim_mean, sim_ci_low, sim_ci_high = mean_ci_t(D_sim_all, confidence=confidence)
...
#     / (n_exp + n_sim - 2)
# )
# cohens_d = (sim_mean - exp_mean) / pooled_sd
# J = 1 - (3 / (4 * (n_exp + n_sim) - 9))
# hedges_g = J * cohens_d

# statistically_distinguishable = p_value < alpha
# interpretation = (
...
# # Save one clean statistical summary table.
# ttest_summary = pd.DataFrame([{
#     "test": "Welch two-sample t-test",
#     "quantity_tested": "temperature-weighted dose, computed as time-integrated flux",
#     "null_hypothesis": "mean experimental dose equals mean simulation dose",
#     "alternative_hypothesis": "means are different",
#     "alpha": alpha,
#     "n_exp": n_exp,
#     "n_sim": n_sim,
#     "D_exp_mean_K_m3": exp_mean,
#     "D_exp_CI95_low_K_m3": exp_ci_low,
#     "D_exp_CI95_high_K_m3": exp_ci_high,
#     "D_exp_std_K_m3": np.std(D_exp_all, ddof=1),
#     "D_sim_mean_K_m3": sim_mean,
#     "D_sim_CI95_low_K_m3": sim_ci_low,
#     "D_sim_CI95_high_K_m3": sim_ci_high,
#     "D_sim_std_K_m3": np.std(D_sim_all, ddof=1),
#     "percent_difference": 100 * (sim_mean - exp_mean) / exp_mean,
#     "welch_t_statistic": t_statistic,
#     "welch_degrees_of_freedom": welch_df,
#     "welch_p_value": p_value,
#     "statistically_distinguishable_alpha_0_05": statistically_distinguishable,
#     "interpretation": interpretation,
#     "cohens_d": cohens_d,
#     "hedges_g": hedges_g,
#     "note": (
#         "Flux histories and experimental measurement uncertainty are reported in the flux plots. "
#         "The t-test is performed on the integrated dose values derived from those flux histories. "
#         "This standard t-test uses the sampled dose distributions and does not directly take propagated uncertainty bars as inputs."
#     ),
# }])

# ttest_summary.to_csv(
#     f"{csv_outdir}/dose_ttest_from_flux_summary_{rs}.csv",
#     index=False,
# )

# print("\nWelch t-test on dose derived from temperature-weighted flux histories:")
# print(ttest_summary)

# # ============================================================
# # Plot: dose means with 95% CI from sample distributions
# # ============================================================

# plot_ci_df = pd.DataFrame({
#     "Source": ["Experiment", "Simulation"],
#     "Mean": [exp_mean, sim_mean],
#     "CI_low": [exp_ci_low, sim_ci_low],
#     "CI_high": [exp_ci_high, sim_ci_high],
# })

# x = np.arange(len(plot_ci_df))
# yerr = np.vstack([
#     plot_ci_df["Mean"].to_numpy() - plot_ci_df["CI_low"].to_numpy(),
#     plot_ci_df["CI_high"].to_numpy() - plot_ci_df["Mean"].to_numpy(),
# ])

# plt.figure(figsize=(6, 4))
# plt.errorbar(
#     x,
#     plot_ci_df["Mean"].to_numpy(),
#     yerr=yerr,
#     fmt="o",
#     capsize=8,
#     linewidth=2.5,
#     markersize=8,
# )
# plt.xticks(x, plot_ci_df["Source"])
# plt.ylabel(r"Temperature-weighted dose, $D_T$ [K m$^3$]")
# plt.title(f"Welch t-test on Integrated Flux Dose: p = {p_value:.2e}")
# plt.tight_layout()
# plt.savefig(
#     f"{outdir}/dose_ttest_from_flux_CI_{rs}.png",
#     dpi=300,
#     bbox_inches="tight",
# )
# plt.close()
...
# # ============================================================
# # Final statistical-analysis console summary
# # ============================================================

# print("\nSaved statistical test outputs:")
# print(f"  {csv_outdir}/dose_ttest_from_flux_summary_{rs}.csv")
# print(f"  {outdir}/dose_ttest_from_flux_CI_{rs}.png")


############## 
# ============================================================
# Uncertainty-aware Welch-style comparison on integrated dose
# ============================================================

alpha = 0.05
n_exp = 3 # len(D_exp_all)
n_sim = 10 #len(D_sim_all)

mean_exp = D_exp_mean_fit
mean_sim = D_sim_mean_fit

# These are the propagated dose uncertainty half-widths used in the interval plot
U_exp = u_D_exp_total
U_sim = u_D_sim_total

# Welch-style comparison using propagated uncertainty as standard uncertainty
se_delta = np.sqrt(U_exp**2 + U_sim**2)

t_unc = (mean_sim - mean_exp) / se_delta

# Conservative effective degrees of freedom
df_unc = min(n_exp - 1, n_sim - 1)

p_unc = 2 * stats.t.sf(np.abs(t_unc), df=df_unc)

uncertainty_ttest_summary = pd.DataFrame([{
    "test": "Uncertainty-aware Welch-style dose comparison",
    "quantity_tested": "temperature-weighted dose with propagated uncertainty",
    "alpha": alpha,
    "n_exp": n_exp,
    "n_sim": n_sim,
    "D_exp_mean_K_m3": mean_exp,
    "D_exp_propagated_uncertainty_K_m3": U_exp,
    "D_exp_low_K_m3": mean_exp - U_exp,
    "D_exp_high_K_m3": mean_exp + U_exp,
    "D_sim_mean_K_m3": mean_sim,
    "D_sim_propagated_uncertainty_K_m3": U_sim,
    "D_sim_low_K_m3": mean_sim - U_sim,
    "D_sim_high_K_m3": mean_sim + U_sim,
    "percent_difference": 100 * (mean_sim - mean_exp) / mean_exp,
    "difference_D_sim_minus_D_exp_K_m3": mean_sim - mean_exp,
    "standard_uncertainty_of_difference_K_m3": se_delta,
    "t_statistic_uncertainty_aware": t_unc,
    "degrees_of_freedom_conservative": df_unc,
    "p_value_uncertainty_aware": p_unc,
    "statistically_distinguishable_alpha_0_05": p_unc < alpha,
    "intervals_overlap": (mean_exp + U_exp) >= (mean_sim - U_sim),
    "interpretation": (
        "Statistically distinguishable at alpha=0.05"
        if p_unc < alpha
    ...
    "note": (
        "This comparison uses the same propagated dose uncertainties shown in the "
        "Hariharan-style interval plot. It is not the same as the sample-only Welch "
        "t-test on the 9 experimental and 30 simulation nominal dose combinations."
    ),
}])

uncertainty_ttest_summary.to_csv(
    f"{csv_outdir}/dose_uncertainty_aware_ttest_summary_{rs}.csv",
    index=False,
)
...
print("\nSaved uncertainty-aware t-test output:")
